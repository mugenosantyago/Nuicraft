package eastonium.mnogii.morph;
 ...
 * {@link eastonium.mnogii.client.PlayerMorphClientEvents}, which is only
...
public class PlayerMorphEventHandler {
    ...
    /**
     * Called from MnogiiPayloads when a MorphRequestPayload arrives.
    ...
    public static void handleMorphRequest(ServerPlayer player, MorphState requested) {
        ...
        MorphState current = player.getData(MnogiiAttachments.MORPH_STATE.get());
    ...
    private static void applyAndBroadcast(ServerPlayer player, MorphState state) {
        player.setData(MnogiiAttachments.MORPH_STATE.get(), state);
