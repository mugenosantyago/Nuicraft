...
def create_animal_entity(entity_id, class_name):
    ...
    const_name = entity_id.upper().replace("_", "")
...
def create_monster_entity(entity_id, class_name):
    """Generate Java entity class for Monster-based entities"""
    const_name = entity_id.upper().replace("_", "")
...
public class Entity{class_name} extends Monster {{
...
MONSTERS = [
    ...
    ('lerahk', 'Lerahk'),
    ('panrahk', 'Panrahk'),
    ('rahkshi_yellow', 'RahkshiYellow'),
    ('turahk', 'Turahk'),
    ('vohrak', 'Vohrak'),
    ('spider_fikou', 'SpiderFikou'),
    ('makuta', 'Makuta'),
]
...
for entity_id, class_name in ANIMALS:
    ...
    print(f"Fixed {class_name} (Animal)")

# Generate monster entities
for entity_id, class_name in MONSTERS:
    filepath = entity_dir / f'Entity{class_name}.java'
    code = create_monster_entity(entity_id, class_name)
    filepath.write_text(code)
    print(f"Fixed {class_name} (Monster)")

print(f"\nFixed {len(ANIMALS) + len(MONSTERS)} entity classes")
